...
func LoadMetaFromMetaFile(baseDir string) (fileMetaMap map[string]*FileMetaData, indexMap map[string]int, e error) {

	// fmt.Println("###-begin to LoadMetaFromMetaFile-###")
	...
	if e != nil || metaFileStats.IsDir() {
		return fileMetaMap, indexMap, nil
	}
	...
	for {
		...
		// fmt.Println("leftOverContent:", leftOverContent)

		currFileMeta := NewFileMetaDataFromConfig(leftOverContent)

		leftOverContent = ""
		fileMetaMap[currFileMeta.Filename] = currFileMeta
		indexMap[currFileMeta.Filename] = index
		index += 1
	}
	// fmt.Println("###-end to LoadMetaFromMetaFile-###")
	return fileMetaMap, indexMap, nil
}
